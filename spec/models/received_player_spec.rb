...
RSpec.describe ReceivedPlayer, type: :model do
  ...
  context 'Valid Attributes' do
    ...
    it '2. It should have a player_name attribute' do
      expect(@receivedPlayer.player_name).to eq(described_class.first.player_name)
    end

    it '3. It should have a player_key attribute' do
      expect(@receivedPlayer.player_key).to eq(described_class.first.player_key)
    end
  ...
  context 'Invalid Attributes' do
    it '1. It should not be able to create a model without a player name' do
      received_player = described_class.create(trade: Trade.first, player_key: '12345')
      expect(received_player).not_to be_valid
    end

    it '2. It should not be able to create a model without a player key' do
      received_player = described_class.create(trade: Trade.first, player_name: 'Test')
      expect(received_player).not_to be_valid
    end

    it '2. It should not be able to create a model without existing inside a trade model' do
      received_player = described_class.create(player_name: Faker::Name.name)
      expect(received_player).not_to be_valid
    end
  end
end
