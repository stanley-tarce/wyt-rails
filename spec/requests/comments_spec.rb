...
RSpec.describe 'Comments', type: :request, :vcr => true do
  ...
  context 'with valid Attributes' do
    ...
    it '2. It should create a comment' do
        ...
        post "/api/create_trade?league_key=#{User.first.leagues.first.league_key}", params: @data, headers: { 'Authorization': "Bearer #{@token}" }
        post "/api/trades/#{User.first.leagues.first.trades.first.id}/comments", params: { comment: {name: "Test", description: "Test 123", trade_id: User.first.leagues.first.trades.first.id}}
    ...
    it '3. It should update a comment' do
        ...
        post "/api/create_trade?league_key=#{User.first.leagues.first.league_key}", params: @data, headers: { 'Authorization': "Bearer #{@token}" }
        post "/api/trades/#{User.first.leagues.first.trades.first.id}/comments", params: { comment: {name: "Test", description: "Test 123", trade_id: User.first.leagues.first.trades.first.id}}
        patch "/api/trades/#{User.first.leagues.first.trades.first.id}/comments/#{User.first.leagues.first.trades.first.comments.first.id}", params: { comment: {name: "Stanley Test", description: "Test 12323233232322"} }
        expect(response).to have_http_status(200)
    end
