class SessionsController < ApplicationController
    ...
    def callback
      if auth_hash
        ...
        TokenHistory.create(user: User.find_by(email: auth_hash['info']['email'] ,access_token: auth_hash['credentials']['token'], refresh_token: auth_hash['credentials']['refresh_token'], expiry: auth_hash['credentials']['expires_at']))
        redirect_to "http://localhost:3000/callback?token=#{Base64.encode64(auth_hash['credentials']['token'])}" unless Rails.env.test?
      end
    end
    def delete
