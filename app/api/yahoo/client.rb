  ...
  class Client
    ...
    def self.leagues(access_token)
      ...
      if response[:code] == 200
        ...
        league_teams = []
        (0...resp_league_teams['count'].to_i).each do |i|
          next unless resp_league_teams[i.to_s]['team'].dig(0, 0, 'team_key')
        ...
        { code: response[:code], response: response[:status], data: { teams: league_teams } }
      else
    ...
    def self.league(access_token, league_key)
      ...
      if response[:code] == 200
        ...
        league = {
          ...
          roster_postions: temp_roster.map do |i|
                             [i['roster_position']['position'], i['roster_position']['count']]
                           end.to_h
        }
        { code: response[:code], response: response[:status], data: { league: league } }
      else
        { code: response[:code], response: response[:status], data: response[:data] }
      end
    end
    ...
    def self.players(access_token, team_key)
      ...
      if response[:code] == 200
        ...
        if resp_players.class != Array
          (0...resp_players['count']).each do |i|
        ...
        { code: response[:code], response: response[:status], data: { players: players } }
      else
        { code: response[:code], response: response[:status], data: response[:data] }
      end
    end
    ...
    def self.teams_in_league(access_token, league_key)
      ...
      if response[:code] == 200
        ...
        league_teams = []
        (0...resp_league_teams['count']).each do |i|
          flattened_team_info = {}
          ...
          league_teams << {
            ...
            manager: flattened_team_info['managers'].map.with_index do |x, j|
                       [j, x['manager']['nickname']]
                     end.to_h
          }
        end
        { code: response[:code], response: response[:status], data: { league_teams: league_teams } }
      else
        { code: response[:code], response: response[:status], data: response[:data] }
      end
    end
    ...
    def self.player_stats(access_token, league_key, player_keys)
      ...
      if response_for_gp[:code] != 200
        { code: response_for_gp[:code], response: response_for_gp[:status],
          data: response_for_gp[:data] }
      else
        # API call for Total Games Played by Player
        resp_player_stats1 = response_for_gp[:data]['fantasy_content']['players']
        player_stats_arr = []
        (0...resp_player_stats1['count']).each do |i|
          flattened_player_stats = {}
          # get player key
          resp_player_stats1[i.to_s]['player'][0].each do |j|
            j.each do |k, v|
              k == 'player_key' ? flattened_player_stats[k] = v : next
            end
          end
          # get games played
          resp_player_stats1[i.to_s]['player'][1]['player_stats']['stats'].each do |x|
            x['stat']['stat_id'] == '0' ? flattened_player_stats['0'] = x['stat']['value'] : next
          end
          player_stats_arr << flattened_player_stats
        end

        # API call for league-specific stats of player
        response_for_stats = Request.call('get',
                                          "/league/#{league_key}/players;player_keys=#{player_keys}/stats?format=json", access_token)
        if response_for_stats[:code] != 200
          return { code: response_for_stats[:code], response: response_for_stats[:status],
                   data: response_for_stats[:data] }
        else
          resp_player_stats2 = response_for_stats[:data]['fantasy_content']['league'][1]['players']
          (0...resp_player_stats2['count']).each do |i|
            resp_player_stats2[i.to_s]['player'][1]['player_stats']['stats'].each do |x|
              player_stats_arr[i][x['stat']['stat_id']] = x['stat']['value']
            end
          end
        end
        ...
        player_stats_arr.each do |x|
          ...
          x.each do |k, v|
            x[k] = Helpers.get_average(x['0'], v) unless %w[5 8 0 1 11 20 26 27 28 player_key].include? k
          end
          # replace stat_id codes with readable abbreviations
          x.transform_keys! { |k| Helpers.stat_categories[k] }
        end

        { code: response_for_stats[:code], response: response_for_stats[:status],
          data: { player_stats: player_stats_arr } }
      end
    end
  end
end
